...
TCPManager::TCPManager(int port, std::string ipAddress, bool client)
{
    if (client)
    {
...
bool TCPManager::Send(char* value, int length)
{
    printf("%s:%d\n", __FUNCTION__,__LINE__);
    if(*value != '\0' || value == NULL ||length == 0)
    {
